...
def read_sql_file(sql_file: Path, logger) -> str:
    try:
        return sql_file.read_text(encoding="utf-8")
    except UnicodeDecodeError:
        logger.info("SQL encoding fallback to cp949: %s", sql_file.name)
        return sql_file.read_text(encoding="cp949")
...
def run(ctx):
    ...
    # --------------------------------------------------
    if ctx.mode == "plan":
        plan_file = ctx.work_dir / "logs" / f"plan_{ctx.run_id}.log"
