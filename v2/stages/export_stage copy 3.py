...
# Export worker
# -------------------------
def _export_one(conn, sql_file, param_set, idx, total_sql, param_idx, total_param, ctx, export_func, fetch_size, compression, stall_seconds, out_dir, host_name, overwrite, backup_keep, ext):
    ...
    if out_file.exists():
        if overwrite:
            ...
            logger.info("%s skip (already completed)", prefix)
            return

    logger.info("%s EXPORT start [%d/%d] param[%d/%d]", prefix, idx, total_sql, param_idx, total_param)
    ...
    logger.info("%s EXPORT done rows=%d size=%.2fMB elapsed=%.2fs", prefix, rows, size_mb, elapsed)
...
def run(ctx):
        ...
        try:
            for t in chunk:
                if stop_event.is_set():
                    logger.warning("Worker-%d stopped by user", worker_id)
                    break
            ...
            try:
                conn.close()
                logger.info("Worker-%d connection closed", worker_id)
            except Exception:
                pass
    ...
    for i, t in enumerate(tasks):
        chunks[i % parallel_workers].append(t)
    ...
    if parallel_workers == 1:
        ...
        with ThreadPoolExecutor(max_workers=parallel_workers) as executor:
            futures = [
                executor.submit(worker, i + 1, chunk)
