...
def init_oracle_client(source_cfg):
    ...
    # logger.info("Oracle client init | lib=%s", lib)
    ...
    if lib:
        try:
            ...
            logger.debug("Oracle Thick mode initialized: %s", lib)
            mode = "thick"
        except Exception as e:
            logger.warning("Thick mode init failed → fallback to Thin mode")
            logger.warning("Reason: %s", e)
    else:
        logger.debug("Instant client not configured → using Thin mode")
    ...
    logger.debug("is_thin_mode(): %s", oracledb.is_thin_mode())
